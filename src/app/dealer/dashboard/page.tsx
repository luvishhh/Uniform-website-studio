...
import { mockUsers, mockOrders, mockProducts } from '@/lib/mockData'
...
import {
  ...
  Eye,
  Percent,
} from 'lucide-react' // Added Percent
...
const mockCustomerData = [
  ...
  {
    id: 'cust_3',
    name: 'Greenwood High (Institution)',
...
const mockPayoutHistory = [
  ...
  { id: 'pay_2', date: '2023-11-01', amount: 380.5, status: 'Paid' },
  { id: 'pay_3', date: '2023-10-01', amount: 520.0, status: 'Paid' },
]

export default function DealerDashboardPage() {
  const router = useRouter()
  const { toast } = useToast()
  const [currentUser, setCurrentUser] = useState<DealerUser | null>(
    defaultDealerForDemo || null
  )
  ...
  useEffect(() => {
    const fetchDealer = () => {
      ...
      if (!activeUser && defaultDealerForDemo) {
        // Fallback to demo if no stored user or user not found
  ...
  useEffect(() => {
    ...
    const fetchDealerDataAndAnalytics = async () => {
      if (currentUser) {
        ...
        const allSalesData = mockOrders.reduce(
        ...
        const pendingCount = mockOrders.filter(
          (order) =>
            ...
            (order.status === 'Awaiting Dealer Acceptance' &&
              order.assignedDealerId === currentUser.id)
        ).length
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const salesThisMonth = mockOrders
        .filter((order) => {
          const orderDate = parseISO(order.orderDate)
          return (
            orderDate.getMonth() === targetMonthDate.getMonth() &&
            orderDate.getFullYear() === targetMonthDate.getFullYear()
          )
        })
        .reduce((sum, order) => sum + order.totalAmount, 0)
    ...
    return (
      <div className='flex flex-col items-center justify-center h-full min-h-[calc(100vh-10rem)] p-6 text-center'>
        ...
        <p className='text-muted-foreground mb-6'>
          Please ensure you are logged in as a dealer or that the default demo
  ...
  const commissionThisMonth = monthlySales * 0.1
    ...
    <div className='space-y-8 p-4 md:p-6'>
      <section className='space-y-6'>
        <Card className='shadow-lg'>
          <CardHeader>
            ...
            <CardDescription>
              Here's an overview of your business on UniShop.
            </CardDescription>
          </CardHeader>
        </Card>
        ...
        <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                ${commissionThisMonth.toFixed(2)}
              </div>
              <p className='text-xs text-muted-foreground'>
                Your estimated 10% earnings
              </p>
            </CardContent>
          </Card>
          <Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest orders and important notifications.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {recentActivity.length > 0 ? (
              ...
              <p className='text-muted-foreground text-center py-4'>
                No recent orders or notifications.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <section>
        ...
        <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <Card className='hover:shadow-lg transition-shadow'>
            ...
            <CardContent>
              <p className='text-sm text-muted-foreground mb-3'>
                View product details and request restocks.
              </p>
              <Button asChild variant='outline' className='w-full'>
          ...
          <Card className='hover:shadow-lg transition-shadow'>
            ...
            <CardContent>
              <p className='text-sm text-muted-foreground mb-3'>
                Manage your dealer account details.
              </p>
              <Button asChild variant='outline' className='w-full'>
      ...
      <div className='grid gap-6 lg:grid-cols-3'>
        <Card className='lg:col-span-2'>
          <CardHeader>
            <CardTitle className='text-xl font-semibold flex items-center'>
              <DollarSign className='mr-2 h-5 w-5 text-primary' /> Sales &amp;
              Revenue
            </CardTitle>
            <CardDescription>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 text-center'>
              ...
              <div className='p-4 bg-muted rounded-lg'>
                ...
                <p className='text-2xl font-bold'>
                  ${(monthlySales * 0.1).toFixed(2)}
                </p>
              </div>
              <div className='p-4 bg-muted rounded-lg'>
                <p className='text-sm text-muted-foreground'>
                  Pending Payouts (Mock)
                </p>
                <p className='text-2xl font-bold'>
                  ${(monthlySales * 0.05).toFixed(2)}
                </p>
              </div>
            </div>
            <div>
            ...
            <div className='flex flex-wrap gap-2'>
              <Button
                ...
                onClick={() =>
                  toast({
                    title: 'Mock Action',
                    description: 'Sales report download initiated (mock).',
                  })
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  toast({
                    title: 'Mock Action',
                    description: 'Tax summary download initiated (mock).',
                  })
                }
              >
                <Download className='mr-2 h-4 w-4' />
        ...
        <Card>
          <CardHeader>
            <CardTitle className='text-xl font-semibold flex items-center'>
              <Users className='mr-2 h-5 w-5 text-primary' /> Customer Overview
            </CardTitle>
            <CardDescription>
              Summary of customers related to your sales.
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            <p className='text-xs text-muted-foreground'>
              (This is a simplified mock view.)
            </p>
            <Table>
              <TableHeader>
                <TableRow>
                    ...
                    <TableCell className='text-right'>
                      <Button
                        ...
                        onClick={() =>
                          toast({
                            title: 'Mock Action',
                            description: `Viewing details for ${customer.name} (mock).`,
                          })
                        }
                      >
            ...
            <Button variant='outline' className='w-full' disabled>
              View All Customers (Coming Soon)
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-xl font-semibold flex items-center'>
            <TrendingUp className='mr-2 h-5 w-5 text-primary' /> Performance
            Insights
          </CardTitle>
          <CardDescription>
            Key metrics and insights for your dealership.
          </CardDescription>
        </CardHeader>
        <CardContent className='space-y-6'>
          <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 text-center'>
      ...
      <section>
        ...
        <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <Card className='opacity-70 cursor-not-allowed'>
            ...
            <CardContent>
              <p className='text-sm text-muted-foreground mb-3'>
                FAQs, guides, and support contact.
              </p>
              <Button variant='outline' className='w-full' disabled>
