...
"use client";
// This page is a copy of the main /products page, but ProductCard will handle purchase restrictions for dealers.
import Header from "@/components/layout/Header"; // We need a public header here, not one from dealer layout
...
export default function DealerProductsPage() {
  ...
  useEffect(() => {
    ...
    // Use router.replace for dealer products to avoid polluting main product page history
    router.replace(`/dealer/products?${params.toString()}`, { scroll: false });
  ...
  const pageTitle = useMemo(() => {
    if (selectedCategory) {
      ...
      return category ? `${category.name} Uniforms` : "Product Catalog";
    }
  ...
  return (
    <>
      {/* Dealer Products page might not use the standard Header if it's part of a dealer-specific layout */}
      {/* For now, assuming it might be a standalone view or part of a simpler dealer layout */}
      <div className="mb-8 text-center mt-2"> {/* Added mt-2 for spacing if Header is removed from DealerLayout */}
        <h1 className="text-4xl font-bold font-headline">{pageTitle}</h1>
        <p className="text-muted-foreground mt-2">Browse our collection of high-quality uniforms for your bulk order needs.</p>
      </div>

      <div className="mb-6 p-4 bg-card rounded-lg shadow-md sticky top-[70px] md:top-[80px] z-30"> {/* Adjusted top sticky value */}
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map(product => (
              <ProductCard key={product.id} product={product} /> // ProductCard will handle role-based button display
            ))}
          </div>
        ) : (
