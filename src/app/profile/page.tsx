...
function ProfileContent() {
  ...
  const getUserDisplayName = (user: User | null): string => {
    ...
    switch (user.role) {
      case 'student':
        return (user as StudentUser).fullName || 'Student'
      case 'institution':
        return (user as InstitutionUser).institutionName || 'Institution'
      case 'dealer':
        return (user as DealerUser).dealerName || 'Dealer'
      case 'admin':
        return (user as AdminUser).email || 'Admin'
      default:
    ...
    if (storedUserId) {
      try {
        ...
        if (userData.role !== 'institution' && userData.role !== 'dealer') {
          ...
          if (!ordersRes.ok) {
            if (ordersRes.status !== 404) {
              // 404 for no orders is acceptable
        ...
        toast({
          ...
          description:
            error.message ||
            'Could not load your profile data. Please try logging in again.',
          variant: 'destructive',
        })
      }
    } else {
  ...
  const handleAvatarUpdate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCurrentUser((prev) =>
          prev ? { ...prev, imageUrl: newAvatarUrl } : null
        )
        toast({
  ...
  if (!currentUser) {
    return (
      <div className='flex flex-col min-h-screen'>
        ...
        <main className='flex-grow container mx-auto px-4 md:px-6 py-8 text-center'>
          <p className='mb-4'>
            Could not load profile information. You might not be logged in.
          </p>
          <Button asChild>
  ...
  const orderProgressStatuses: OrderStatus[] = [
    'Placed',
    'Confirmed',
          ...
          <Card className='md:w-1/4 lg:w-1/5 h-fit sticky top-24 shadow-lg'>
            <CardHeader className='items-center text-center relative'>
              ...
              <CardDescription className='text-sm'>
                {displayIdentifier} ({currentUser.role})
              </CardDescription>
            </CardHeader>
            <CardContent className='p-3'>
              ...
              <Button
                variant='ghost'
                className='w-full justify-start text-destructive hover:text-destructive/90 hover:bg-destructive/10 py-2.5'
              ...
              <TabsContent value='details'>
                <Card className='shadow-lg'>
                  ...
                  <CardContent className='space-y-3 text-sm'>
                    ...
                    <p>
                      <strong>Primary Identifier:</strong> {displayIdentifier}
                    </p>
                    <p>
                      <strong>Role:</strong>{' '}
                    ...
                    {currentUser.role === 'student' && (
                      <>
                        ...
                        <p>
                          ...
                          {(currentUser as StudentUser).schoolCollegeName}
                        </p>
                        <p>
                          <strong>Grade/Course:</strong>{' '}
                          {(currentUser as StudentUser).gradeOrCourse}
                        </p>
                        {(currentUser as StudentUser).year && (
                          <p>
                            <strong>Year:</strong>{' '}
                            {(currentUser as StudentUser).year}
                          </p>
                        ...
                        <p>
                          ...
                          {(currentUser as StudentUser).parentName}
                        </p>
                        <p>
                          <strong>Parent Contact:</strong>{' '}
                          {(currentUser as StudentUser).parentContactNumber}
                        </p>
                        {(currentUser as StudentUser).address && (
                          <>
                            ...
                            <p>
                              {(currentUser as StudentUser).address?.street}
                            </p>
                            <p>
                              {(currentUser as StudentUser).address?.city},{' '}
                              {(currentUser as StudentUser).address?.zip},{' '}
                              {(currentUser as StudentUser).address?.country}
                            </p>
                          </>
                        )}
                      </>
                      ...
                      <>
                        ...
                        <p>
                          ...
                          <span className='capitalize'>
                            {(currentUser as InstitutionUser).institutionType}
                          </span>
                        </p>
                        <p>
                          <strong>Contact Number:</strong>{' '}
                          {(currentUser as InstitutionUser).contactNumber}
                        </p>
                        <p>
                          <strong>Address:</strong>{' '}
                          {
                            (currentUser as InstitutionUser)
                    ...
                    {currentUser.role === 'dealer' && (
                      <>
                        ...
                        <p>
                          <strong>Contact Number:</strong>{' '}
                          {(currentUser as DealerUser).contactNumber}
                        </p>
                        <p>
                          <strong>Business Address:</strong>{' '}
                          {(currentUser as DealerUser).businessAddress}
                        </p>
                        <p>
                          <strong>GSTIN:</strong>{' '}
                          {(currentUser as DealerUser).gstinNumber}
                        </p>
                      </>
                    )}
                                        ...
                                        (statusStep, index, arr) => {
                                          ...
                                          return (
                                            <React.Fragment key={statusStep}>
                                              ...
                                              {index < arr.length - 1 && (
                                                <div
                                                  className={`flex-1 h-1 min-w-[20px] ${
                                                    isCompleted &&
                                                    currentStatusIndex > index
                                                      ? 'bg-primary'
                                                      : 'bg-border'
              ...
              <TabsContent value='settings'>
                <Card className='shadow-lg'>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Manage your account preferences (e.g., password change -
                      mock).
                    </CardDescription>
                  </CardHeader>
      ...
      >
        <DialogContent className='sm:max-w-md'>
          ...
          <div className='py-4 space-y-4'>
            ...
            <div>
              <Label htmlFor='avatarFileInput' className='text-sm'>
              ...
              <p className='text-xs text-muted-foreground mt-1'>
                Max file size: 2MB. Recommended formats: JPG, PNG.
              </p>
            </div>
          </div>
