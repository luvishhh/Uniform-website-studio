...
const DONATION_FEATURE_LS_KEY = 'unishop_donation_feature_enabled' // Defined the key
...
export default function HomePage() {
  ...
  // Check initial state and listen for storage changes
  useEffect(() => {
    if (isClient) {
  ...
  return (
    <div className='flex flex-col min-h-screen'>
      ...
      <main className='flex-grow'>
        ...
        <section className='relative w-full h-[60vh] sm:h-[70vh] md:h-[85vh] lg:h-screen min-h-[350px] overflow-hidden group'>
          {/* Background Carousel */}
          <Carousel
            opts={{ loop: true }}
          ...
          >
            ...
            <div className='absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20'>
              <CarouselPrevious className='bg-background/70 hover:bg-background text-foreground border-border hover:border-primary h-9 w-9 sm:h-10 sm:w-10 md:h-12 md:w-12' />
            </div>
            <div className='absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20'>
              <CarouselNext className='bg-background/70 hover:bg-background text-foreground border-border hover:border-primary h-9 w-9 sm:h-10 sm:w-10 md:h-12 md:w-12' />
            </div>
            <CarouselDots className='absolute bottom-4 sm:bottom-6 md:bottom-8 z-20' />
          </Carousel>

          <div className='absolute inset-0 flex flex-col justify-center items-center p-3 sm:p-8 md:p-16 text-center text-white z-10 pointer-events-none'>
            <div className='max-w-xs sm:max-w-xl md:max-w-2xl lg:max-w-3xl bg-black/60 backdrop-blur-lg p-4 sm:p-8 md:p-10 rounded-xl shadow-2xl pointer-events-auto border border-white/10'>
              <h1 className='text-2xl sm:text-4xl md:text-6xl font-bold font-headline mb-4 sm:mb-6 leading-tight md:leading-snug'>
                Dress for Success. Discover UniShop.
              </h1>
              <p className='text-base sm:text-lg md:text-xl lg:text-2xl mb-6 sm:mb-8 opacity-95'>
                Premium quality school, corporate, and healthcare uniforms
                designed for comfort and durability.
              </p>
              <Button
                size='lg'
                className='w-full sm:w-auto bg-accent hover:bg-accent/90 text-accent-foreground shadow-lg hover:shadow-primary/30 transition-all duration-300 transform hover:scale-105 px-6 sm:px-10 py-3 sm:py-4 text-base sm:text-lg'
                asChild
              >
                <Link
                  href='/products'
        ...
        <section className='py-6 sm:py-8 md:py-12 bg-background border-y border-border/60'>
          <div className='container mx-auto px-2 sm:px-4 md:px-6 text-center mb-4 sm:mb-6 md:mb-8'>
            <h2 className='text-xs sm:text-base font-semibold text-muted-foreground uppercase tracking-wider'>
        ...
        {/* Why UniShop Section - Improved for Mobile */}
        <section className='py-8 sm:py-10 md:py-24 bg-muted/30'>
          <div className='container mx-auto px-2 sm:px-4 md:px-6'>
            <h2 className='text-2xl sm:text-4xl md:text-5xl font-bold font-headline text-center mb-3 sm:mb-4'>
              Why <span className='text-primary'>UniShop</span>?
            </h2>
            <p className='text-base sm:text-lg text-muted-foreground text-center mb-6 sm:mb-10 md:mb-16 max-w-xl sm:max-w-2xl mx-auto'>
              Your trusted partner for quality, convenience, and community.
            </p>
            <div className='grid gap-5 sm:gap-8 md:gap-10 md:grid-cols-3 text-center'>
              {[
                {
                  icon: ShoppingBag,
                  title: 'Quality First',
                  description:
                    'Durable materials and comfortable designs that last.',
                },
                {
                  icon: Shirt,
                  title: 'Perfect Fit',
                  description:
                    'Wide range of sizes and styles for a smart, professional look.',
                },
                {
                  icon: Zap,
                  title: 'Easy Shopping',
                  description:
                    'User-friendly online store with secure and fast checkout.',
                },
              ].map((item, index) => (
                <div
                  key={index}
                  className='flex flex-col items-center p-4 sm:p-6 md:p-8 bg-card rounded-xl shadow-lg border border-transparent hover:border-primary/50 hover:shadow-xl transition-all duration-300 transform hover:scale-105'
                >
                  <item.icon className='h-10 w-10 sm:h-14 sm:w-14 text-primary mb-4 sm:mb-5' />
                  <h3 className='text-lg sm:text-2xl font-semibold font-headline mb-2 sm:mb-3'>
                    {item.title}
                  </h3>
        ...
        <section className='bg-background'>
          <div className='container mx-auto px-4 md:px-6 py-10 md:py-24'>
            <h2 className='text-4xl md:text-5xl font-bold font-headline text-center mb-10 md:mb-16'>
            ...
            <section className='py-16 md:py-24 bg-accent text-accent-foreground'>
              <div className='container mx-auto px-4 md:px-6 text-center'>
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href='/donate'>Learn About Donations</Link>
                </Button>
              </div>
            </section>
          )}
