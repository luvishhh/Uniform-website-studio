...
const defaultInstitutionForDemo = mockUsers.find(
  (u) => u.id === 'inst_1' && u.role === 'institution'
) as InstitutionUser | undefined
const defaultProductsForDemo = defaultInstitutionForDemo
  ? mockProducts.filter(
      (p) =>
        p.institution &&
        defaultInstitutionForDemo.institutionName &&
        p.institution.toLowerCase() ===
          defaultInstitutionForDemo.institutionName.toLowerCase()
    )
  : []
...
export default function InstitutionDashboardPage() {
  ...
  const [currentUser, setCurrentUser] = useState<InstitutionUser | null>(
    defaultInstitutionForDemo || null
  )
  const [institutionProducts, setInstitutionProducts] = useState<Product[]>(
    defaultProductsForDemo
  )
  ...
  const [purchasingStudentsDetails, setPurchasingStudentsDetails] = useState<
    Array<StudentUser & { purchasedItems: CartItem[]; totalSpent: number }>
  >([])
      ...
      if (storedUserId && storedUserRole === 'institution') {
        ...
        try {
          ...
          if (userRes.ok) {
              ...
              setInstitutionProducts(
                mockProducts.filter(
                  (p) =>
                    ...
                    p.institution.toLowerCase() ===
                      realInstitution.institutionName.toLowerCase()
                )
              )
              console.log(
                'Successfully loaded real institution data:',
                realInstitution.institutionName
              )
            } else {
              // Logged-in user is not an institution. Revert to demo data if it was set.
              console.warn(
                `User ID ${storedUserId} is not an institution role. Reverting to demo data if available.`
              )
            ...
            console.warn(
              `Failed to fetch user data for ID: ${storedUserId}, status: ${userRes.status}. Reverting to demo data if available.`
            )
            if (defaultInstitutionForDemo) {
              setCurrentUser(defaultInstitutionForDemo)
              setInstitutionProducts(defaultProductsForDemo)
          ...
          console.warn(
            'Error fetching logged-in institution from API. Reverting to demo data if available.',
            error
          )
          if (defaultInstitutionForDemo) {
            setCurrentUser(defaultInstitutionForDemo)
        ...
        if (defaultInstitutionForDemo) {
          console.log(
            'No real institution logged in. Displaying demo data for Greenwood High.'
          )
        } else {
          console.error(
            "CRITICAL: Default institution 'inst_1' NOT FOUND in mockUsers. Dashboard will be empty."
          )
          toast({
            title: 'Dashboard Load Error',
            description:
              'Default institution data missing. Please check mock data.',
            variant: 'destructive',
          })
        }
        setIsLoading(false)
    ...
    if (
      ...
      const studentsOfInstitution = mockUsers.filter(
        (u): u is StudentUser =>
          ...
          (u as StudentUser).schoolCollegeName.toLowerCase() ===
            institutionNameLower
      )
      setRegisteredStudents(studentsOfInstitution)

      let purchasingStudentsMap = new Map<
        string,
        { student: StudentUser; purchasedItems: CartItem[]; totalSpent: number }
      >()
      ...
      mockOrders.forEach((order) => {
        ...
        order.items.forEach((item) => {
          ...
          if (
            ...
            productSaleCounts[item.productId] =
              (productSaleCounts[item.productId] || 0) + item.quantity
          }
        })
        ...
        if (orderContainsInstitutionProduct) {
          ...
          if (
            orderUser &&
            orderUser.role === 'student' &&
            typeof (orderUser as StudentUser).schoolCollegeName === 'string' &&
            (orderUser as StudentUser).schoolCollegeName.toLowerCase() ===
              institutionNameLower
          ) {
            ...
            if (existingEntry) {
              ...
              existingEntry.totalSpent += orderItemsForInstitution.reduce(
              ...
              purchasingStudentsMap.set(studentData.id, {
                ...
                totalSpent: orderItemsForInstitution.reduce(
                  (sum, cartItem) => sum + cartItem.price * cartItem.quantity,
                  0
                ),
              })
      ...
      const detailedPurchasers = Array.from(purchasingStudentsMap.values()).map(
        (entry) => ({
          ...entry.student,
      ...
      if (Object.keys(productSaleCounts).length > 0) {
        const mostSoldProductId = Object.keys(productSaleCounts).reduce(
          (a, b) => (productSaleCounts[a] > productSaleCounts[b] ? a : b)
        )
        const mostSoldProductDetails = mockProducts.find(
          (p) => p.id === mostSoldProductId
        )
        if (mostSoldProductDetails) {
          topProduct = {
            name: mostSoldProductDetails.name,
            quantity: productSaleCounts[mostSoldProductId],
          }
        }
      }
  ...
  const institutionSalesData = useMemo(() => {
    ...
    const monthlySales: Record<
      string,
      { sales: number; revenue: number; date: Date }
    > = {}
    ...
    mockOrders.forEach((order) => {
      ...
      if (dateRange?.from && dateRange?.to) {
        validDateRange = isWithinInterval(orderDate, {
      ...
      order.items.forEach((item) => {
        ...
        if (
          ...
          if (!salesByProduct[productDetails.name]) {
            salesByProduct[productDetails.name] = {
      ...
      <div className='flex items-center justify-center h-full min-h-[calc(100vh-10rem)]'>
        ...
        <div>
          ...
          <p className='text-muted-foreground'>
            Please ensure you are logged in or try again later. If this
            persists, default demo data might be missing.
          </p>
    ...
    <div className='space-y-8'>
      <div className='bg-card p-6 rounded-lg shadow-md'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-muted-foreground'>
              This is your central hub for managing uniforms and orders.
            </p>
          </div>
        </div>
      </div>
      ...
      <section>
        ...
        <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <Card>
            <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
              <CardTitle className='text-sm font-medium'>
                Students Purchased Uniforms
              </CardTitle>
              <UserCheck className='h-4 w-4 text-muted-foreground' />
            </CardHeader>
            <CardContent>
              <div className='text-2xl font-bold'>
                {purchasingStudentsDetails.length}
              </div>
              <p className='text-xs text-muted-foreground'>
                Unique students who bought uniforms
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
              <CardTitle className='text-sm font-medium'>
                Total Uniform Orders
              </CardTitle>
              <ShoppingCart className='h-4 w-4 text-muted-foreground' />
            </CardHeader>
            <CardContent>
              <div className='text-2xl font-bold'>
                {totalUniformOrdersCount}
              </div>
              <p className='text-xs text-muted-foreground'>
                Orders containing your institution's uniforms
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                {mostPopularProduct.quantity} units sold
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                Unique uniform products for your institution
              </p>
            </CardContent>
          </Card>
        </div>
      ...
      <section className='space-y-6'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Monthly units sold and revenue from your institution's specific
              uniforms.
            </CardDescription>
          </CardHeader>
          <CardContent className='h-[350px] flex items-center justify-center'>
            {institutionSalesData.length > 0 ? (
              ...
              <p className='text-muted-foreground text-center'>
                No sales data available for the selected date range or
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Breakdown of units sold per uniform type for your institution.
            </CardDescription>
          </CardHeader>
          <CardContent className='h-[400px] flex items-center justify-center'>
            {productSalesDistribution.length > 0 ? (
              ...
              <p className='text-muted-foreground text-center'>
                No product sales data available for this institution or date
                range.
              </p>
            )}
          </CardContent>
      ...
      <section className='space-y-6'>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Registered Students from {currentUser?.institutionName}
            </CardTitle>
            <CardDescription>
              List of students registered with your institution.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {registeredStudents.length > 0 ? (
                        ...
                        <div className='flex items-center gap-3'>
                          <Avatar className='h-9 w-9'>
                            <AvatarImage
                              src={
                                student.imageUrl ||
                                `https://placehold.co/40x40.png?text=${getInitials(
                                  student.fullName
                                )}`
                              }
              ...
              <p className='text-muted-foreground text-center py-8'>
                No students found registered for {currentUser?.institutionName}.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Students Who Purchased Uniforms from{' '}
              {currentUser?.institutionName}
            </CardTitle>
            <CardDescription>
              Details of students from your institution who purchased uniforms.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {purchasingStudentsDetails.length > 0 ? (
              ...
              <p className='text-muted-foreground text-center py-8'>
                No students from {currentUser?.institutionName} have purchased
                uniforms yet for the selected period.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <section>
        <h2 className='text-2xl font-bold font-headline mb-6'>
          Uniforms for {currentUser?.institutionName || 'Your Institution'}
        </h2>
        {institutionProducts.length > 0 ? (
          ...
          <Card className='text-center py-12'>
            <CardContent>
              ...
              <p className='text-muted-foreground'>
                No specific uniforms are currently listed for{' '}
                {currentUser?.institutionName || 'your institution'}.
                <br />
