...
export async function POST(request: Request) {
  try {
    ...
    if (
      !role ||
      !['student', 'institution', 'dealer', 'admin'].includes(role)
    ) {
      return NextResponse.json(
    ...
    if (useMockData()) {
      ...
      const exists = mockUsers.find((u) => {
        ...
        if (role === 'student')
          return (u as StudentUser).rollNumber === userData.rollNumber
        return u.email === userData.email
      })
      if (exists) {
        return NextResponse.json(
          { message: `${identifierField} already exists for this role.` },
          { status: 409 }
        )
      }
      // Simulate registration
      const hashedPassword = await bcrypt.hash(password, 10)
      let newUserObject: Omit<User, 'id' | '_id'> & { cart: CartItem[] }
      switch (role) {
        case 'student':
          newUserObject = {
            ...userData,
      ...
      return NextResponse.json(
        {
          ...
          user: { ...userWithoutPassword, id: mockId },
        },
        { status: 201 }
      )
    }
    ...
    switch (role) {
      case 'student':
        if (
          !userData.schoolCollegeName ||
          !userData.fullName ||
          !userData.institutionType ||
          !userData.gradeOrCourse ||
          !userData.parentName ||
          !userData.parentContactNumber
        ) {
          return NextResponse.json(
      ...
      case 'institution':
        if (
          ...
          !userData.institutionType ||
          !userData.institutionalAddress ||
          !userData.contactNumber
        ) {
          return NextResponse.json(
            { message: 'Missing required institution fields' },
            { status: 400 }
          )
        }
        newUserObject = {
