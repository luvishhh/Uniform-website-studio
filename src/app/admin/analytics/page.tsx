...
import { mockProducts, mockOrders, mockCategories } from '@/lib/mockData'
...
export default function AdminAnalyticsPage() {
  ...
  const displayedSalesData = useMemo(() => {
    let dataToDisplay = fullSalesData.map((d) => ({
      ...d,
      name: format(d.date, 'MMM yy'),
    }))
    if (dateRange?.from) {
    ...
    mockOrders.forEach((order) => {
      order.items.forEach((item) => {
        ...
        if (
          ...
          (productDetails.category === 'School' ||
            productDetails.category === 'College')
        ) {
          salesByInstitution[productDetails.institution] =
            (salesByInstitution[productDetails.institution] || 0) +
            item.quantity
        }
      })
    })
    return Object.entries(salesByInstitution)
      .map(([institutionName, quantitySold]) => ({
      ...
      <div className='space-y-6'>
        <Card>
          <CardHeader className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
            <div>
              ...
              <CardDescription>
                Monthly sales and revenue. Select a date range to filter.
              </CardDescription>
            </div>
            <Popover>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Total uniform units sold (School and College), broken down by
              institution.
            </CardDescription>
          </CardHeader>
          <CardContent className='h-[400px] flex items-center justify-center'>
            {institutionSalesDistributionData.length > 0 ? (
                ...
                <PieChart>
                  <Pie
                    ...
                    label={({ name, percent, value }) =>
                      `${name}: ${value} units sold (${(percent * 100).toFixed(
                        0
                      )}%)`
                    }
                  >
              ...
              <p className='text-muted-foreground text-center'>
                No sales data by institution available.
              </p>
            )}
          </CardContent>
        </Card>
