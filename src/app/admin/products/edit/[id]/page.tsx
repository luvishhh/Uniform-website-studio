...
export default function AdminEditProductPage() {
  ...
  const [formData, setFormData] = useState<
    Partial<Product> & { sizes?: string; colors?: string }
  >({})
  ...
  const handleSelectChange = (name: string, value: string) => {
    ...
    if (name === 'category') {
      setFormData((prev) => ({
        ...prev,
        category: value as 'School' | 'College',
      }))
    }
  }
    ...
    toast({
      ...
      action: (
        <Button
          variant='outline'
          size='sm'
          onClick={() => console.log('Undo mock update')}
        >
          Undo
        </Button>
      ),
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className='space-y-6'>
            ...
            <div className='grid sm:grid-cols-2 gap-6'>
              ...
              <div>
                <Label htmlFor='productInstitution'>
                  School/College Name (Institution)
                </Label>
                <Input
                  id='productInstitution'
                  name='institution'
            ...
            <div className='grid sm:grid-cols-1 gap-6'>
              {' '}
              {/* Changed to single column as stock is removed */}
              <div>
