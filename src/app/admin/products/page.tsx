...
const getUniqueValues = (products: any[], key: string) => {
  if (key === 'category') {
    return Array.from(new Set(products.map((p: any) => p.category))).sort()
  }
  return Array.from(
    new Set(products.map((p: any) => p[key]).filter(Boolean))
  ).sort()
}
...
function AdminProductsContent() {
  ...
  const allInstitutions = getUniqueValues(products, 'institution')
  const allGenders: ('Unisex' | 'Boys' | 'Girls' | string)[] = [
    'Unisex',
    'Boys',
    'Girls',
  ]
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedInstitution !== 'all') {
      filtered = filtered.filter(
        (p: any) => p.institution === selectedInstitution
      )
    }
