  ...
  return (
    <Card className="overflow-hidden h-full flex flex-col group bg-card shadow-md hover:shadow-xl transition-all duration-300 ease-in-out rounded-lg border border-transparent hover:border-primary/30 focus-within:border-primary/50">
      <CardHeader className="p-0 relative">
        <Link href={`/products/${product.id}`} className="block aspect-square relative w-full overflow-hidden rounded-t-lg">
          <Image
          ...
          {product.featured && !isAdminView && product.stock > 0 && (
            <Badge variant="default" className="absolute top-3 left-3 z-10 bg-accent text-accent-foreground">Featured</Badge>
          )}
          {isAdminView && (
            <Badge variant="secondary" className="absolute top-3 left-3 z-10">Stock: {product.stock}</Badge>
          )}
        </Link>
      </CardHeader>
      <CardContent className="p-4 flex-grow flex flex-col">
        <div className="flex-grow">
          ...
          <Link href={`/products/${product.id}`} className="focus:outline-none">
            <h3 className="text-base font-semibold font-headline leading-snug group-hover:text-primary transition-colors line-clamp-2 mb-1 focus:underline">
              {product.name}
            </h3>
          </Link>
